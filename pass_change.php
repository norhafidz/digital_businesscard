<?php 
    ...
    // execute forwarded to index if no user associated with the id.
       header('location:index.php');
        ...
        $newpassconfirm = mysqli_real_escape_string($connection, $_POST['newpassconfirm']);
        $current = mysqli_real_escape_string($connection, $_POST['current']);

        // check POST variables not equal to null

        if (($memorable == '') or (!isset($memorable)) or ($newpass == '') or (!isset($newpass)) or ($newpassconfirm == '') or (!isset($newpassconfirm)) or ($current  == '') or (!isset($current))) {
        ...
        if ($memorable != $memorable_fromDB) {
            ...
            if ($current != $current_fromDB) {
                ...
                mysqli_query($connection, "UPDATE users SET password='$newpass' where id='$user_id'");
